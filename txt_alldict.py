...
def get_worddict():
    dic={}
    afs=open("./txt字典.txt",'r',encoding='gb18030')
...
def get_cipinworddict():
    dic={}
    afs=open("./txt词频字典.txt",'r',encoding='gb18030')
    for line in afs.readlines():
        key=line.split(":")[0].strip()
        value=int(line.split(":")[-1].strip())
        dic[key]=value
...
def get_class_cipinworddict(name_temp):
    dic={}
    afs=open("./txt解词\\"+name_temp+'\\词频'+name_temp+'.txt','r',encoding='gb18030')
    for line in afs.readlines():
        key=line.split(':')[0].strip()
        value=int(line.split(':')[-1].strip())
        dic[key]=value
...
def create_feature_file():
    ...
    N=getN()
    #print(N)
    ofs=open("./txt_featurefile.txt",'w',encoding="gb18030")
    temp_class=''
    for fullname in fullname_list:
        str=''
        classno = -1
        dirname = fullname.split("./txt解词\\")[1].split('\\')[0]
